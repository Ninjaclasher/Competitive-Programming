...
int id[2000010], st[2000010], arr[1000005];
int n, k;
...
static inline __attribute__((always_inline)) void update(int x, int v)
{
    st[x+=n] = v;
    for (x >>= 1; x; x >>= 1)
...
static inline __attribute__((always_inline)) void update2(int x)
{
    x+=n;
    for (x >>= 1; x; x >>= 1)
        id[x] = arr[id[x<<1]] > arr[id[x<<1|1]] ? id[x<<1] : id[x<<1|1];
...
static inline __attribute__((always_inline)) bool replace(int l, int x, int r)
{
    return arr[x] >= arr[l] && arr[x] > arr[r];
}

static inline __attribute__((always_inline)) void run(int x)
{
